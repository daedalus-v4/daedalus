import { Channel, GuildMember, Invite, Message, PartialMessage, PermissionFlagsBits, escapeMarkdown } from "discord.js";
import { DbAutomodSettings, escapeRegExp } from "shared";
import { code, englishList } from "../../lib/format.js";

export type AutomodRule = DbAutomodSettings["rules"][0];

const invitePattern = new RegExp(Invite.InvitesPattern, "g");
const cache = new Map<string, Message[]>();

export function skipScan(message: Message | PartialMessage, rule: AutomodRule, settings: DbAutomodSettings) {
    if (!message.member || skipScanForMember(message.member, rule, settings)) return true;
    ...
    while (channel) {
        ...
        if (!rule.disregardDefaultIgnoredChannels && settings.ignoredChannels.includes(channel.id)) return true;
...
function skipScanForMember(member: GuildMember, rule: AutomodRule, settings: DbAutomodSettings) {
    ...
    if (member.roles.cache.hasAny(...settings.ignoredRoles)) return true;

    return rule.onlyWatchEnabledRoles;
}

export async function match(rule: AutomodRule, message: Message, multiDeleteTargets: Message[]): Promise<[string, string] | undefined> {
    const { type } = rule;
    ...
    if (type === "blocked-terms") {
        ...
        if (caps <= rule.capsSpamData.limit || (caps * 100) / letters.length < rule.capsSpamData.ratioLimit) return;
        ...
        for (const line of lines) {
            ...
            if (line.length > 0) consecutive = 0;
        }
        ...
        return [
            `Your message contained ${[
                maxConsecutive > consecutiveLimit ? "more than the allowed consecutive newlines" : [],
                lines.length - 1 > totalLimit ? "more than the allowed newlines" : [],
            ]
                .flat()
                .join(" and ")}.`,
        ...
        return [
            `Your message contained more than the allowed number of repeated characters in a row.`,
        ...
        const variant = { ratelimit: "messages", "attachment-spam": "files", "sticker-spam": "stickers", "link-spam": "links", "mention-spam": "mentions" }[
            type
        ];
