...
import { DbGiveawaysSettings } from "shared";
import { db } from "shared/db.js";

export async function countEntries(giveaway: DbGiveawaysSettings["giveaways"][0], member: GuildMember) {
    if (
...
export async function draw(guild: Guild, giveaway: DbGiveawaysSettings["giveaways"][0], winners?: number) {
    winners ??= giveaway.winners;

    const entries = await db.giveawayEntries.find({ guild: guild.id, id: giveaway.id }).toArray();

    const tickets: Record<string, { count: number; display: string }> = {};
    let total = 0;

    for (const { user: id } of entries) {
