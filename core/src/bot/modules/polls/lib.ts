import { BaseMessageOptions, ButtonComponentData, ButtonStyle, ComponentType, Guild } from "discord.js";
import { DBPoll } from "shared";
import { getColor } from "shared/db.js";

export async function renderPoll(poll: DBPoll, guild: Guild): Promise<BaseMessageOptions> {
    const tally: Record<string, number> = {};
    for (const vote of Object.values(poll.votes)) for (const item of Array.isArray(vote) ? vote : [vote]) tally[item] = (tally[item] ?? 0) + 1;
    ...
    if (poll.type === "yes-no" || poll.type === "binary") {
        ...
        const count = Object.values(poll.votes).filter((vote) => vote.some((x) => poll.options.includes(x))).length;
