...
import { Channel, GuildBasedChannel, Message, PartialMessage } from "discord.js";
...
export default (app: Argentium) => app;
...
async function checkStars(input: Message | PartialMessage) {
    ...
    const target = await getStarboardLink(starboard.target, message);
    ...
    if (count < starboard.threshold) {
        ...
        if (target) await target.edit({ content });
            ...
            if (stars.has(message.id)) return setTimeout(() => checkStars(message), 1000);
            ...
            let embeds = [
                {
                    ...
                    image: single ? { url: attachments[0].attachment as string } : null,
                },
            ];
            ...
            if (!message.embeds.some((embed) => embed.data.type === "rich")) files = single ? [] : attachments;
                ...
                embeds[0].image = null;
                files = attachments;
                embeds = [...message.embeds.map((embed) => embed.toJSON()).slice(0, 9), embeds[0]];
            }
...
async function getStarboardLink(channel: Channel, message: Message) {
    if (!channel.isTextBased()) return;

    const entry = await db.starboardLinks.findOne({ message: message.id });
    if (!entry) return;
