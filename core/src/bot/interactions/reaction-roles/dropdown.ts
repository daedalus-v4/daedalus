...
export default async function (cmd: StringSelectMenuInteraction) {
    ...
    if (entry.type === "lock") {
        ...
        else return template.success(`Set your selected role${add.length === 1 ? "" : "s"} from this prompt to ${englishList(add.map((x) => `<@&${x}>`))}.`);
    } else if (entry.type === "verify") {
        await member.roles.add([...new Set(add)]);
        return template.success(`Added ${englishList(add.map((x) => `<@&${x}>`))} to you.`);
    }
}
